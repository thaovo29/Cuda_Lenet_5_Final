...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    std::cout << "Constant-shared memory, split layer" << std::endl;
    ...
    int W_grid = ceil(float(W_out) / TILE_WIDTH_C1); 
        ...
        // smem_size
        int smem_size = C * (TILE_WIDTH_C1 + K - 1) * (TILE_WIDTH_C1 + K - 1) * sizeof(float);
        ...
        //launch the kernel
        conv_forward_kernel_c1<<<gridSize, blockSize, smem_size>>>(device_y, device_x, device_k, B, M, C, H, W, K);
    } 
    else {
    int H_grid = ceil(float(H_out) / TILE_WIDTH_C3);
    int W_grid = ceil(float(W_out) / TILE_WIDTH_C3); 
        ...
        // smem_size
        int smem_size = C * (TILE_WIDTH_C3 + K - 1) * (TILE_WIDTH_C3 + K - 1) * sizeof(float);
        ...
        //launch the kernel
        conv_forward_kernel_c3<<<gridSize, blockSize, smem_size>>>(device_y, device_x, device_k, B, M, C, H, W, K);
    }
